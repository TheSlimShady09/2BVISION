  ...
  if (user?.role !== 'admin') {
    return (
      <div className="min-h-[50vh] flex items-center justify-center bg-zinc-50">
        <div className="text-[#707070] font-light text-lg">Unauthorized Access</div>
      </div>
    );
  }
    ...
    <div className="w-full px-4 sm:px-6 lg:px-8 bg-zinc-50 min-h-screen py-12 border-t border-zinc-200">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-8 border border-zinc-200 shadow-sm">
          <div>
            <h1 className="text-3xl font-bold text-[#2d2d2d] mb-1 uppercase tracking-widest">Admin Control</h1>
            <p className="text-[#707070] font-light">Manage studio operations and content.</p>
          </div>
        </div>
          ...
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 pb-4 text-sm font-bold uppercase tracking-widest transition-colors ${
                activeTab === tab.id ? 'text-[#2d2d2d] border-b-2 border-[#1e1e1e]' : 'text-slate-400 hover:text-[#555555]'
              }`}
            >
          ...
          >
            ...
            {activeTab === 'bookings' && (
              <div className="space-y-6">
                <h2 className="text-2xl font-bold text-[#2d2d2d] mb-6">Booking Management</h2>
                {bookings.length === 0 ? <p className="text-[#707070]">No bookings found.</p> : (
                    ...
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-zinc-100">
                        {bookings.map(booking => (
                          <tr key={booking.id} className="text-sm text-[#444444]">
                            <td className="py-4 pr-4 font-medium text-[#2d2d2d]">
                            ...
                            <td className="py-4 pr-4"><span className="bg-zinc-100 px-2 py-1 text-xs font-bold uppercase">{booking.package_id}</span></td>
                            <td className="py-4">
                              <select 
                      ...
                      <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-zinc-100">
                          {portfolioItems.map(item => (
                            <tr key={item.id} className="text-sm text-[#444444]">
                              ...
                              <td className="py-3 pr-4 font-medium text-[#2d2d2d]">{item.title}</td>
                              <td className="py-3 pr-4">
                                <span className="bg-zinc-100 px-2 py-1 text-xs font-bold uppercase">{item.type}</span>
                              </td>
                              <td className="py-3 pr-4 text-[#707070]">{item.category}</td>
                              <td className="py-3">
                                <button
            ...
            {activeTab === 'messages' && (
              <div className="space-y-6">
                <h2 className="text-2xl font-bold text-[#2d2d2d] mb-6">Contact Messages</h2>
                {messages.length === 0 ? <p className="text-[#707070]">No messages found.</p> : (
                  <div className="space-y-4">
                    {messages.map(msg => (
                      <div key={msg.id} className="border border-zinc-200 p-6 bg-zinc-50">
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            <h4 className="font-bold text-[#2d2d2d]">{msg.name}</h4>
                            <p className="text-sm text-[#707070]">{msg.email}</p>
                          </div>
                          <span className="text-xs text-slate-400 font-bold uppercase">{new Date(msg.created_at).toLocaleDateString()}</span>
                        </div>
                        <p className="text-[#444444] font-light leading-relaxed">{msg.message}</p>
                      </div>
                    ))}
                  </div>
                )}
            ...
            {activeTab === 'deliverables' && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div>
                  <h2 className="text-2xl font-bold text-[#2d2d2d] mb-6">Assign Deliverable</h2>
                  <form onSubmit={handleDeliverableSubmit} className="space-y-4">
                    ...
                    <div>
                      ...
                      <select value={deliverableForm.type} onChange={e => setDeliverableForm({...deliverableForm, type: e.target.value})} className="w-full bg-zinc-50 border border-zinc-200 p-3 text-[#2d2d2d] outline-none focus:border-[#2d2d2d]">
                    ...
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-[#707070] mb-2">Deliverable URL</label>
                      <input required type="url" value={deliverableForm.url} onChange={e => setDeliverableForm({...deliverableForm, url: e.target.value})} className="w-full bg-zinc-50 border border-zinc-200 p-3 text-[#2d2d2d] outline-none focus:border-[#2d2d2d]" />
                    </div>
                    <button type="submit" className="w-full py-4 bg-[#2d2d2d] text-white font-bold tracking-widest uppercase text-sm hover:bg-[#1e1e1e] transition-all mt-4">
                      Assign to Client
                    </button>
                ...
                <div>
                  <h2 className="text-2xl font-bold text-[#2d2d2d] mb-6">Recent Deliverables</h2>
                  <div className="space-y-4">
                    {deliverables.map(del => (
                      <div key={del.id} className="border border-zinc-200 p-4 flex items-center justify-between">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <span className="bg-zinc-100 px-2 py-1 text-xs font-bold uppercase tracking-widest text-[#555555]">{del.type}</span>
                          </div>
                          <p className="text-sm font-medium text-[#2d2d2d]">{del.bookings?.event_type}</p>
                        </div>
                        <a href={del.url} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-[#2d2d2d] transition-colors">
