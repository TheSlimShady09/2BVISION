    ...
    <section id="pricing" className="min-h-screen bg-[#1e1e1e] py-24 px-4 sm:px-6 lg:px-8 border-t border-[#383838]">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 items-center mb-16">
        {packages.map((pkg, index) => {
          const prefix = pkg.id.split('-')[0];
          const name = t(`pricing.${prefix}Name`);
          const description = t(`pricing.${prefix}Desc`);
          const features = t(`pricing.${prefix}Feat`);
            ...
            >
              ...
              <div className="mb-8">
                <div className="flex items-center justify-between mb-4">
                  <h3 className={`text-2xl font-bold ${pkg.popular ? 'text-[#2d2d2d]' : 'text-white'}`}>{name}</h3>
                  {pkg.icon}
                </div>
                <p className={`text-sm h-10 ${pkg.popular ? 'text-[#555555]' : 'text-[#a0a0a0]'}`}>{description}</p>
              </div>
